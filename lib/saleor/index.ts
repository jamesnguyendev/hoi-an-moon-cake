...
export async function createCart(billingInfo: string): Promise<Cart> {
  const billingData = JSON.parse(billingInfo);

  const formattedPhone = billingData.phone.startsWith('+')
    ? billingData.phone
    : `+84${billingData.phone.replace(/^0/, '')}`;
    ...
    variables: {
      input: {
        ...
        billingAddress: {
          ...
          streetAddress1: billingData.address,
          phone: '84901234567',
